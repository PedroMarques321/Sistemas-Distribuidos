...
struct entry_t **rtable_get_table(struct rtable_t *rtable) {
    ...
    if (msg_in->opcode == MESSAGE_T__OPCODE__OP_ERROR) {
        printf("Erro: server enviou opcode ERROR do rtable_get_table");
        message_destroy(msg_in);
        return NULL;
    }
...
struct statistics_t *rtable_stats(struct rtable_t *rtable) {
    ...
    // Definir o tipo de operação
    msg_out->opcode = MESSAGE_T__OPCODE__OP_STATS;
    msg_out->c_type = MESSAGE_T__C_TYPE__CT_NONE;
    ...
    if (msg_in->opcode == MESSAGE_T__OPCODE__OP_ERROR) {
        printf("Erro: server enviou opcode ERROR do rtable_stats");
        message_destroy(msg_in);
        return NULL;
    }

    if (msg_in->opcode != MESSAGE_T__OPCODE__OP_STATS + 1) {
        printf("Erro: resposta com opcode inesperado");
        printf("OP_CODE recebido: %d", msg_in->opcode);
        message_destroy(msg_in);
        return NULL;
    ...
    statistics = malloc(sizeof(struct statistics_t));
    statistics->client_n = msg_in->clients;
    statistics->op_n = msg_in->operations;
    statistics->time = msg_in->time;
    message_destroy(msg_in);
    printf("rtable_stats: Sucesso\n");
    return statistics;
}
