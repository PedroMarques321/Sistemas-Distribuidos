...
int is_same_server(struct rtable_t *rtable, char *server_address_port1) {
    if (rtable == NULL || server_address_port1 == NULL) return 0;
    char server_address_port[50] = "", tmpStr[10];
    strcpy(server_address_port, rtable->server_address);
    strcat(server_address_port, ":");
    sprintf(tmpStr, "%d", rtable->server_port);
    strcat(server_address_port, tmpStr);
    return strcmp(server_address_port, server_address_port1);
}


int main(int argc, char **argv) {
    char *zk_address_port;
    int arguments_ok = 1;
    int i;
    int iaddrn[5] = {0};
    ...
    zk_address_port = argv[1];

    // inicializar zookeeper
    client_zk_connect(zk_address_port);

    // estrtuturas rtable para ligações ao HEAD e ao TAIL
    ...
    while (1) {
        ...
        // verificar se estamos ligados ao HEAD
        if (rtable_head == NULL) {
            printf("Ligar ao HEAD\n");
            rtable_head = rtable_connect(get_head_server_address());
            if (rtable_head == NULL) {
                printf("ERRO: rtable_connect HEAD\n");
                return -1;
            }
        }
        else if(is_same_server(rtable_head, get_head_server_address())) {
            printf("HEAD antigo: %s\n", rtable_head->server_address);
            printf("HEAD novo: %s\n", get_head_server_address());
            printf("Ligar ao novo HEAD\n");
            if (rtable_disconnect(rtable_head) == -1) {
                printf("ERRO: rtable_disconnect HEAD\n");
                return -1;  // mas assim não se liga de novo ao HEAD caso o HEAD antigo tenha morrido
            }
            rtable_head = rtable_connect(get_head_server_address());
            if (rtable_head == NULL) {
                printf("ERRO: rtable_connect HEAD\n");
        ...
        // verificar se estamos ligados ao TAIL
        if (rtable_tail == NULL) {
            printf("Ligar ao TAIL\n");
            rtable_tail = rtable_connect(get_tail_server_address());
            if (rtable_tail == NULL) {
                printf("ERRO: rtable_connect TAIL\n");
                return -1;
            }
        }
        else if(is_same_server(rtable_tail, get_tail_server_address())) {
            printf("TAIL antigo: %s\n", rtable_tail->server_address);
            printf("TAIL novo: %s\n", get_tail_server_address());
            printf("Ligar ao novo TAIL\n");
            if (rtable_disconnect(rtable_tail) == -1) {
                printf("ERRO: rtable_disconnect TAIL\n");
                return -1;  // mas assim não se liga de novo ao TAIL caso o TAIL antigo tenha morrido
            }
            rtable_tail = rtable_connect(get_tail_server_address());
            if (rtable_tail == NULL) {
                printf("ERRO: rtable_connect TAIL\n");
        ...
        if (strcmp(cp, "get") == 0) {
            ...
            if (key == NULL) {
                ...
                block = rtable_get(rtable_tail, key);
                if(block == NULL) {
        ...
        if (strcmp(cp, "getkeys") == 0) {
            keys = rtable_get_keys(rtable_tail);
            if (keys == NULL) {
        ...
        if (strcmp(cp, "gettable") == 0) {
            table = rtable_get_table(rtable_tail);
            if (table == NULL) {
        ...
        if(strcmp(cp, "stats") == 0) {
            statistics = rtable_stats(rtable_tail);
            if (statistics == NULL) {
        ...
        // Comando QUIT
        if (strcmp(cp, "quit") == 0) {
            if(rtable_disconnect(rtable_head) != 0 || rtable_disconnect(rtable_tail) != 0) {
                return -1;
            }
            return 0;
        }
